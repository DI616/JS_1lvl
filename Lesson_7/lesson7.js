...
var obstacle_timer // Таймер для создания препятствия
var score = 0; // Результат
var disScore = document.getElementById("score");
...
function startGame() {
    ...
    obstacle_timer = setInterval(createObstacle, 10000);
    setTimeout(createFood, 100);
}
/**
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !isSnakeObstacle(new_unit)) {
        ...
        if (direction == 'x-') {
            new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-'/*19*/ + (FIELD_SIZE_X - 1))[0];
        }
        else if (direction == 'x+') {
...
function removeObstacle() {
    var obstacleRemoved = document.getElementsByClassName('obstacle-unit');
    if (obstacleRemoved.length == 0) {
        return;
    }
    else {
        var removed = obstacleRemoved[0].getAttribute('class').split(' ');
        obstacleRemoved[0].setAttribute('class', removed[0] + ' ' + removed[1]);
    }
}
/**
